...
void *sensor_thread(void *arg)
{
    ...
    printf("[TRANS] Sensor thread started\n");

    while (!shutdown_flag) {
        int   gear   = (rand() % 6) + 1;
        float torque = 50.0f + (float)(rand() % 200);

        pthread_mutex_lock(&sync_data.mutex);
        sync_data.data.gear   = gear;
        sync_data.data.torque = torque;
...
void *can_thread(void *arg)
{
    ...
    printf("[TRANS] CAN thread started\n");
    ...
    while (!shutdown_flag) {
        ...
        pack_transmission(&frame, local.gear, local.torque);
...
void *fault_thread(void *arg)
{
    ...
    printf("[TRANS] Fault thread started\n");

    while (!shutdown_flag) {
        if (inject_fault) {
            pthread_mutex_lock(&sync_data.mutex);
            sync_data.data.gear   = 0;
            sync_data.data.torque = 0.0f;
            pthread_mutex_unlock(&sync_data.mutex);

            printf("[TRANS] Fault injected: gear=0 torque=0\n");
            inject_fault = 0;
        }
        usleep(100000);
    }
...
void *limp_thread(void *arg)
{
    ...
    printf("[TRANS] Limp thread started\n");

    while (!shutdown_flag) {
        if (fd >= 0 && read_limp_command(fd, &cmd) == 0) {
            if (cmd.ecu_id == ECU_ID)
                printf("[TRANS] Limp mode active: limit=%.0f\n", cmd.limit);
        }
        usleep(100000);
    }
...
int main(void)
{
    ...
    bus = can_shm_attach();
    if (!bus) {
        fprintf(stderr, "[TRANS] Failed to attach shared memory\n");
        return EXIT_FAILURE;
    }

    if (can_sem_open() < 0) {
        fprintf(stderr, "[TRANS] Failed to open semaphore\n");
        return EXIT_FAILURE;
    }
    ...
    printf("[TRANS] Shutdown complete\n");
    return 0;
}
